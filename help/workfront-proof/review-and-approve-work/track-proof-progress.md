...
<<<<<<< Updated upstream
TQID: https://experienceleague.adobe.com/wzltSslylibcXyHEtIeCnNFLbxeQCyFkafWErbg81MU
product_v2:
...
**Once a final decision** (e.g., Approved or Rejected) is made on a proof, all SOCD indicators may appear green for users in that stage, even if individual actions (such as opening the proof or making comments) were not performed. This is a system-wide behavior designed to reflect overall stage completion, not individual engagement.

**Prior to a decision being registered**, each SOCD indicator reflects actual user activity (e.g., white if no action was taken, green if the action was completed). After the decision, the system assumes workflow completion and updates all indicators accordingly.
