...
You should brainstorm with your team and identify any risks to the project. For risks where you can estimate the cost if the risk occurs and the probability that it will occur, be sure and enter these values. Workfront will multiply the potential cost by the probability and put that in a [!UICONTROL Potential Risk] fund that will be subtracted from the project's [!UICONTROL Planned Benefit] when calculating its [!UICONTROL Net Value]. 
...
The [!UICONTROL Business Case] is flexible. You can fill out just a few sections or none at all. None of the fields are required. However, the more information you fill out, the easier it is to analyze and prioritize projects vying for the same budget or resources.  

Once the [!UICONTROL Business Case] is filled out, click the **[!UICONTROL Submit]** button in the summary panel on the right side of the window. This will change the project status to [!UICONTROL Requested]. Now you're ready to use [!UICONTROL Portfolio Optimization] to prioritize projects in the same portfolio. 

>[!NOTE]
>
>The alignment score in the [!UICONTROL Business Case] summary panel is generated when you fill out the scorecard. The alignment score is one of the values used in calculating the [!UICONTROL Portfolio Optimizer] score. 
